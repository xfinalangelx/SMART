...
export default function ClinicsScreen() {
  ...
  async function fetchClinics() {
    try {
      setLoading(true);
      const { data, error } = await supabase.rpc('get_all_clinic');
      
      if (error) {
        console.error('Error fetching clinics:', error);
        Alert.alert(
          language === 'bm' ? 'Ralat' : 'Error',
          language === 'bm' ? 'Gagal memuatkan data klinik' : 'Failed to load clinics'
        );
      } else {
        setClinics(data || []);
      }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Text style={styles.title}>
        {language === 'bm' ? 'Senarai Klinik' : 'List of Clinics'}
      </Text>
