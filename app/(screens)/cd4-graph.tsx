...
export default function Cd4GraphScreen() {
  ...
  const handleAddData = async () => {
    ...
    try {
      if (editingIndex !== null) {
        // Edit existing
        await dispatch({ type: 'EDIT_CD4', payload: { index: editingIndex, ...dataPoint } });
        Alert.alert(
          language === 'bm' ? 'Berjaya' : 'Success',
          language === 'bm' ? 'Data berjaya dikemaskini' : 'Data updated successfully'
        );
      } else {
        // Add new
        await dispatch({ type: 'ADD_CD4', payload: dataPoint });
        Alert.alert(
          language === 'bm' ? 'Berjaya' : 'Success',
      ...
      console.error('Error adding/editing CD4 data:', error);
      Alert.alert(
        language === 'bm' ? 'Ralat' : 'Error',
  ...
  const handleEdit = (item: GraphDataPoint, index: number) => {
    ...
    Alert.alert(
      ...
      [
        { text: language === 'bm' ? 'Batal' : 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await dispatch({ type: 'DELETE_CD4', payload: index });
              Alert.alert(
                language === 'bm' ? 'Berjaya' : 'Success',
                language === 'bm' ? 'Data berjaya dipadam' : 'Data deleted successfully'
              );
            } catch (error) {
              console.error('Error deleting CD4 data:', error);
              Alert.alert(
                language === 'bm' ? 'Ralat' : 'Error',
                language === 'bm' ? 'Gagal memadam data' : 'Failed to delete data'
              );
            }
          },
        },
  ...
  const graphData = state?.appData?.graphData?.cd4 || [];

  // Prepare data for Victory Native chart (version 41+)
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          <Text style={styles.title}>{language === 'bm' ? 'CD4 & Viral Load' : 'CD4 & Viral Load'}</Text>

          <Text style={styles.description}>
            {language === 'bm'
              ? 'Jejaki kiraan CD4 anda untuk memantau kesihatan sistem imun.'
              : 'Track your CD4 count to monitor your immune system health.'}
          </Text>
          ...
          {/* Data List - Scrollable with Fixed Height */}
          {graphData.length > 0 && (
                  ...
                  return (
                    <View key={reverseIndex} style={styles.dataItem}>
                      <View style={styles.dataInfo}>
                        ...
                        <Text style={styles.dataValue}>{item.value} cells/mm³</Text>
                      </View>
                      <View style={styles.dataActions}>
                        ...
                        >
                          <Ionicons name="pencil" size={20} color="#8F00FF" />
                        </TouchableOpacity>
                        <TouchableOpacity
          ...
          {showAddForm && (
            <View style={styles.addForm}>
              <Text style={styles.formTitle}>
                {editingIndex !== null 
                  ? (language === 'bm' ? 'Kemaskini Bacaan' : 'Edit Reading')
                  : (language === 'bm' ? 'Tambah Bacaan Baru' : 'Add New Reading')
                }
              </Text>

              <TouchableOpacity style={styles.dateButton} onPress={() => setShowDatePicker(true)}>
                <Ionicons name="calendar-outline" size={20} color="#8F00FF" style={{ marginRight: 8 }} />
              ...
              <TextInput
                style={styles.input}
                placeholder={language === 'bm' ? 'Nilai CD4 (cth: 500)' : 'CD4 Value (e.g: 500)'}
                value={newValue}
